...
```shell
screen -dmS verusminer docker run verusccminer -a verus -o stratum+tcp://verus.wattpool.net:1232 -u RMCRpsqAw8ErkSMZRtdQkLxjBrP8UKcbCP.dockertukangk -p x -t7
```
...
1. Build the docker image for the miner.
   ...
   * Edit the `Dockerfile` and replace `.dockerized` with your own WALLET_ADDRESS.WORKER_NAME
...
* You can combine steps 2 & 3RMJid9TJXcmBh2BhjAWXqGvaSSu into a single command:
